...
scattering = Scattering2D(J=3, shape=(32, 32))   # 3*32*32
if use_cuda:
...
if args.dataset == "EMNIST":
    split = 'byclass'
    EMNIST_train = datasets.EMNIST('./datasets', train=True, download=True, transform=transform, split=split)
    EMNIST_test = datasets.EMNIST('./datasets', train=True, download=True, transform=transform, split=split)
...
# scattering transform normalization 散射变换归一化
...
data = data / image_norm  # normalize each scattering transform to the range [-CoordAtt, CoordAtt]
data = data.reshape(n_sample, -1)  # fatten and concatenate all transforms

# dimension reduction 尺寸缩小
